/*
	Rollup.js v0.64.1
	Tue, 14 Aug 2018 16:58:29 GMT - commit 285ab760e75c69a5fbeae6758c9089656f475438
...
function createAssetPluginHooks(assetsById, watchFiles, outputBundle, assetFileNames) {
    return {
        emitAsset: function (name, dependenciesOrSource, source) {
            ...
            var asset = { name: name, source: source, fileName: undefined, dependencies: dependencies, transform: null };
            ...
            if (!asset.transform && asset.dependencies && asset.dependencies.length) {
                for (var _a = 0, _b = asset.dependencies; _a < _b.length; _a++) {
                    var depId = _b[_a];
                    watchFiles[depId] = true;
                }
            }
            return assetId;
        },
        setAssetSource: function (assetId, dependenciesOrSource, source) {
            ...
            if (Array.isArray(dependenciesOrSource)) {
                ...
                asset.dependencies = dependenciesOrSource.map(function (depId) { return normalize(path.resolve(depId)); });
            }
            else {
                source = dependenciesOrSource;
            }
...
function createTransformEmitAsset(assetsById, emitAsset) {
    ...
    return {
        assets: assets,
        emitAsset: function (name, dependenciesOrSource, source) {
            var assetId = emitAsset(name, dependenciesOrSource, source);
            var asset = assetsById.get(assetId);
...
function transformChunk(graph, chunk, code, sourcemapChain, options) {
    var transformChunkAssetPluginHooks = createAssetPluginHooks(graph.assetsById, graph.watchFiles);
    var transformChunkReducer = function (code, result, plugin) {
        if (result == null)
            return code;
        if (typeof result === 'string') {
            ...
            throw new Error((inTransformBundle ? 'transformBundle' : 'transformChunk') + " must return a \"map\" sourcemap property when sourcemaps are enabled.");
        }
    ...
    return graph.pluginDriver
        .hookReduceArg0('transformChunk', [code, options, chunk], transformChunkReducer, function (pluginContext) { return (__assign({}, pluginContext, transformChunkAssetPluginHooks)); })
        .then(function (code) {
        inTransformBundle = true;
        return graph.pluginDriver.hookReduceArg0('transformBundle', [code, options, chunk], transformChunkReducer, function (pluginContext) { return (__assign({}, pluginContext, transformChunkAssetPluginHooks)); });
    })
        .catch(function (err) {
        error(err, {
        ...
        if (isNamespaceVariable(traced.variable)) {
            ...
            for (var _i = 0, _a = Object.keys(namespaceVariables); _i < _a.length; _i++) {
                ...
                if (original.included) {
                    ...
                    var importTrace = this.traceExport(importName, traced.variable.module);
...
function createPluginDriver(graph, options, pluginCache, watcher) {
    ...
    var _a = createAssetPluginHooks(graph.assetsById, graph.watchFiles), emitAsset = _a.emitAsset, getAssetFileName$$1 = _a.getAssetFileName, setAssetSource = _a.setAssetSource;
...
function transform(graph, source, module) {
    ...
    function transformReducer(code, result, plugin) {
        ...
        if (customTransformCache) {
            ...
            for (var _b = 0, assets_1 = assets; _b < assets_1.length; _b++) {
                var asset = assets_1[_b];
                if (asset.dependencies) {
                    for (var _c = 0, _d = asset.dependencies; _c < _d.length; _c++) {
                        var depId = _d[_c];
            ...
            if (result && Array.isArray(result.dependencies)) {
                if (!transformDependencies)
                    transformDependencies = [];
                for (var _e = 0, _f = result.dependencies; _e < _f.length; _e++) {
                    var dep = _f[_e];
    ...
    return graph.pluginDriver
        ...
        return __assign({}, pluginContext, { cache: trackedPluginCache ? trackedPluginCache.cache : pluginContext.cache, warn: function (warning, pos) {
            ...
            emitAsset: emitAsset, setAssetSource: function () {
                return error({
            ...
            if (cachedModule &&
                ...
                if (cachedModule.transformAssets) {
                    for (var _i = 0, _a = cachedModule.transformAssets; _i < _a.length; _i++) {
                        var asset = _a[_i];
                        _this.pluginDriver.emitAsset(asset.name, (asset.dependencies ? asset.dependencies : asset.source), asset.dependencies && asset.source);
                    }
                }
                return cachedModule;
            }
        ...
        return graph_1.pluginDriver
            ...
            function generate(rawOutputOptions, isWrite) {
                ...
                return createAddons(graph_1, outputOptions)
                    ...
                    var generateAssetPluginHooks = createAssetPluginHooks(assets, graph_1.watchFiles, outputBundle, assetFileNames);
                    return graph_1.pluginDriver
